...
\footnotesize
  ...
  \begin{tabular}{llll}
    ...
    lambda\_step                  &               & (many)           & size of the wavelength pixel for the rectified spectra \\
...
This is the last module, in which the brightest object in each slit is identified and its 1D spectrum is extracted. Optimal extraction is used, but setting \code{extract\_optimal=0} forces boxcar extraction. When using optimal extraction, the weights are determined by fitting a Gaussian function to the observed profile (i.e., the 2D spectrum collapsed along the wavelength direction). However, for bright targets it may be better to derive the weights directly from the observed profile; this can be achieved setting \code{extract\_gaussian\_profile=0}.
...
A simple boxcar extraction can be performed manually on a 2D spectrum output by \flame\ using the provided IDL routine \code{slame\_util\_extract\_spectrum}, for example like this:
\begin{lstlisting}
  flame_util_extract_spectrum, 'slit07.fits', $
    yrange=[52,58]
\end{lstlisting}
Here, \code{yrange} is an integer array specifying the first and last pixel rows (in detector coordinates) to be considered for the extraction. The 1D spectrum is then saved as a FITS file in the same format as for the files output by \code{flame\_extract}.
