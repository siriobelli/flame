...
\item \emph{(optional)} Create an ASCII file with the names of the flat field frames (e.g., flats.txt)
\item \emph{(optional)} Create an ASCII file with the names of the dark frames (e.g., darks.txt)
\end{itemize}
...
input.darks_filelist = 'darks.txt'
...
The last required input is the y pixel position of a reference star in both the A and the B position. These coordinates do not need to be exact. If the trace is very faint in a single frame, a simple A-B subtraction is generally sufficient for a visual detection. The A and B coordinates need to be specified as the two elements of an array, for example:
\begin{lstlisting}
input.startrace_y_pos = [547, 560]
\end{lstlisting}
...
darks\_filelist   		& none 				& name of ASCII file containing the list of dark frames\\
flats\_filelist   		& none 				& name of ASCII file containing the list of flat field frames	\\
...
startrace\_y\_pos 		& [0, 0]			& pixel y-coordinate of the A and B position of the star trace \\
...
xrange\_star 			& [1000, 1200]		& range of star trace (in pixel x-coordinate) that will be extracted \\
wavecal\_approx\_smooth 	& 5 				& smoothing length used in the approximate wavecal calibration \\
