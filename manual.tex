...
\footnotesize
  ...
  \begin{tabular}{llll}
    ...
    darksub\_data                 & 1             & calibrations     & subtract the master dark (if provided) by the science data \\
...
\begin{figure}[tbp]
\centering
\includegraphics[width=0.7\textwidth]{oversample_05.png}
\includegraphics[width=0.7\textwidth]{oversample_1.png}
\includegraphics[width=0.7\textwidth]{oversample_2.png}
\caption{Aaa.}
...
Finally, this module identifies the brightest object in each slit and extracts the 1D spectrum. Output files are saved in the directory \code{extraction/}, which is created inside the output directory. These include, for each slit, a ps file showing the spatial profile with its Gaussian fit, the extracted spectrum, and the SNR for both the boxcar extraction and the optimal extraction. The 1D spectrum extracted from each slit is stored in a FITS file as a structure containing the fields \code{lambda}, \code{flux}, and \code{ivar}, corresponding to wavelength (NB: in angstrom), flux (in electron per second), and inverse variance. The error spectrum can be easily obtained as $\mathrm{err} = 1/\sqrt{\mathrm{ivar}}$.
